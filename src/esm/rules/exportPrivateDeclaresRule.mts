  ...
  create(context) {
    ...
    return {
      ClassDeclaration(declaration) {
        ...
        if (parentDeclaration.parent !== null ||
          parentDeclaration.type !== 'Program' ||
          !parentDeclaration.comments?.length) {
          return;
        }
        const hasPrivateAnnotatedComment = parentDeclaration.comments.some((s) => (
        ...
        if (!hasPrivateAnnotatedComment ||
          parentDeclaration.range === undefined ||
      ...
      FunctionDeclaration(declaration) {
        ...
        const hasPrivateAnnotatedComment = parentDeclaration.comments.some((s) => s.value.includes('@private') && 
          s.range?.[1] !== undefined &&
          // @ts-expect-error
          s.range?.[1] === ( declaration.start || 0) - 1,
        );
      ...
      ExportNamedDeclaration(node) {
        ...
        if (declaration != null) {
          ...
          if (declaration.type === 'FunctionDeclaration') {
            context.report({
              ...
              fix: (fixer) => exportRestrictFixer<typeof declaration.type>(fixer, node, { declaration }),
            });
          ...
          const variableIdentifiers = declaration.declarations
            .map(s => s.id.type === 'Identifier' ? s.id : undefined).filter(s => s !== undefined);
        ...
        const mappedIdentifiers = node.specifiers.map((s) => {
          if (!!s.exported && s.exported.type === 'Identifier') {
            return s.exported;
          }
          return undefined;
        }).filter((s): s is ESTree.Identifier => s !== undefined);
        const findIdentifiers = mappedIdentifiers.filter(s => restrictedExportsInfo.some(t => (
        ...
        findIdentifiers.forEach((identifier, index) =>{
          ...
          context.report({
            ...
            fix: (fixer) => exportRestrictFixer<typeof findTypes[0]>(fixer, node, { identifier }),
          });
        });
      },
    };
